...
def run_batadal_automata_noise_metrics(config: dict) -> dict:
...
def main() -> None:
    ...
    report = run_batadal_automata_noise_metrics(config)

    output_path = Path("reports/results/automata_noise_metrics_batadal.json")
    output_path.parent.mkdir(parents=True, exist_ok=True)

    with output_path.open("w", encoding="utf-8") as file:
        json.dump(report, file, indent=2, ensure_ascii=False)

    print("BATADAL automata noise metric evaluation completed.")
