...
def run_single_skab_noise_fold(
    df,
    ...
    if not no_group_leakage:
        raise ValueError(f"Group leakage detected in SKAB noise fold {fold_index}.")
    ...
    noisy_scaled_test_df = add_gaussian_noise(
        df=scaled_test_df,
        feature_columns=feature_columns,
...
def run_skab_automata_noise_metrics(config: dict) -> dict:
    skab_config = config["datasets"]["skab"]
    preprocessing_config = config["preprocessing"]
    automata_config = config["automata"]
    noise_config = config["experiments"]["noise"]
    ...
    for fold_index, (train_indices, test_indices) in enumerate(folds, start=1):
        fold_report = run_single_skab_noise_fold(
            df=df,
            ...
            random_seed=config["project"]["random_seeds"][0] + fold_index,
            fold_index=fold_index,
        )
    ...
    report = {
        ...
        "noise": {
            "mean": noise_config["mean"],
            "std": noise_config["std"],
            "applied_to": "scaled_test_features",
        },
        "target_column": skab_config["target_column"],
...
def main() -> None:
    ...
    report = run_skab_automata_noise_metrics(config)

    output_path = Path("reports/results/automata_noise_metrics_skab.json")
    output_path.parent.mkdir(parents=True, exist_ok=True)

    with output_path.open("w", encoding="utf-8") as file:
        json.dump(report, file, indent=2, ensure_ascii=False)

    print("SKAB automata noise metric evaluation completed.")
